...
export default function Register({ onRegister }: RegisterTypeProps) {
    ...
    async function handleSubmit(e: React.SubmitEvent<HTMLFormElement>) {
        ...
        try {
            const response = await httpClient.Fetch<number>({
                path: 'Auth/Register',
                method: "POST",
            ...
            if (response.res?.ok) {
                ...
                setMessage(`Đăng ký thất bại: ${response.res.status} ${response.res.statusText} - ${response.body?.message}`);
            }
            else {
                setMessage('Đăng ký thất bại: Không thể kết nối đến server');
            }
        }
        catch (e) {
            setMessage(`Đăng ký thất bại: ${e}`);
        }
    }
